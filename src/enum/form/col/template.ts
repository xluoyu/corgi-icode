...
export default function(formOptions: IFormItemOptions, widget: IFormComp, options: { hasValidate: any }) {
  ...
  const children = widget.children
    ? widget.children.map((child) => {
      ...
      if (child.noForm) {
        ...
        return `<el-form-item label="${form.label.value}"${options.hasValidate ? ` prop="${form._key.value}"` : ''}>
