  ...
  <el-aside class="bg-$theme-bg w-1/6 px-4">
    <el-tabs v-model="activeTab" tab-position="top" class="h-full">
      <el-tab-pane
        ...
        :label="$t('page.componentConfig')"
        name="comp"
      >
        <keep-alive>
          <RenderForm
            :key="curActionWidget.key"
            :form-options="formOptionsToArray(curActionWidget.form)"
            :form-data="curActionWidget.form"
            @update="updateActionWidgetOptions"
          />
        </keep-alive>
      </el-tab-pane>
      <el-tab-pane :label="$t('page.formConfig')" name="form">
        <RenderForm
          :form-options="formOptionsToArray(formOptions)"
          @update="updateFormOptions"
        />
      </el-tab-pane>
    </el-tabs>
  </el-aside>
